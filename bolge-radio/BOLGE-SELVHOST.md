...
**Hele mappen skal op på GitHub** — ikke kun filerne i toppen. På [github.com/k-papa/BolgeRadio](https://github.com/k-papa/BolgeRadio) mangler lige nu `src/`, `public/`, `scripts/` og `server/`. Uden dem kan Vercel ikke bygge appen.

På computer (anbefalet):

1. Pak zip’en ud, så du ser mapperne `src`, `public`, `scripts`, `server`
2. I GitHub: **Add file → Upload files**
3. Træk **hele den udpakkede mappe** ind (alle undermapper med)
4. Commit til `main`
5. Tjek at `src/components/radio/app.tsx` findes på GitHub, før du deployer
...
- Node.js: **22.x**
- Output Directory: **lad feltet stå tomt**
- Du behøver **ingen** `DATABASE_URL`

Advarslerne `npm warn deprecated recharts` og `eslint` er harmløse. De er ikke fejlen.
